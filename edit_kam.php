...
<body class="app sidebar-mini">
    ...
    <div class="container" style="margin-top:20px">
		<h2>Edit Kadar Air</h2>
		
		<hr>
		
		<?php
		//jika sudah mendapatkan parameter GET id_ph_air dari URL
		if(isset($_GET['id_ph_air'])){
			//membuat variabel $id_ph_air untuk menyimpan id_ph_air dari GET id_ph_airdi URL
			$id_ph_air= $_GET['id_ph_air'];
			
			//query ke database SELECT tabel ph_air berdasarkan id_ph_air= $id_ph_air
			$select = mysqli_query($koneksi, "SELECT * FROM ph_air WHERE id_ph_air='$id_ph_air'") or die(mysqli_error($koneksi));
			
			//jika hasil query = 0 maka muncul pesan error
			if(mysqli_num_rows($select) == 0){
				echo '<div class="alert alert-warning">id_ph_airtidak ada dalam database.</div>';
				exit();
			//jika hasil query > 0
			}else{
		...
		if(isset($_POST['submit'])){
			...
			$sql = mysqli_query($koneksi, "UPDATE ph_air SET ph_before='$ph_before', ph_after='$ph_after', waktu_perbaikan='$waktu_perbaikan' WHERE id_ph_air='$id_ph_air'") or die(mysqli_error($koneksi));
			
			if($sql){
				echo '<script>alert("Berhasil mengubah data."); document.location="kadar_air_minum.php?id='.$id_ph_air.'";</script>';
			}else{
		...
		<form action="edit_kam.php?id_ph_air=<?php echo $id_ph_air; ?>" method="post">
